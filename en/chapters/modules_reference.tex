...
\todo{Choose the modules to remove from this list.}
...
Listing \refandpage{lst:ref_fstab_sample} shows a sample tree of the Fstab lens.

\subsection{Examples of path expressions}

\augtoolcode|match '/files/etc/fstab/*[label() != "#comment"]'|

matches all lines except comment lines.

\augtoolcode|match '/files/etc/fstab/*[file = "/"]/spec'|

matches the \verb!spec! node for the filesystem mounted on ``/''.

\augtoolcode|rm '/files/etc/fstab/*/opt[. = "nosuid"]'|
